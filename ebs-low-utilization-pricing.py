...
with open('underutilized_ebs_vol.csv', 'w') as csvfile:
            writer = csv.writer(csvfile, delimiter=',')
            writer.writerow(['Region','Volume ID','Size','Volume Status', 'Status', 'Volume Type','Count','Savings'])
    ...
    for volume in volumes:
        ...
        for day in daystomonitor:
            ...
            if not ebsreadresults:
                ...
                print 'hell'
        if count == 7:
                with open('underutilized_ebs_vol.csv', 'a') as csvfile:
                    writer = csv.writer(csvfile, delimiter=',')
                    writer.writerow([region.name,volume.id,volume.size,volume.status, 'Underutilized', volume.type,count,float(volume.size*0.12)])
        else:
               print "Hello"
